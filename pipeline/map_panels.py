...
for ax in axes:
    ...
    ax.set_axis_off()
_rho = open(ROOT / "reports/v4/usgs_validation.txt").read().split("Spearman(ours, USGS): ")[1].strip()
fig.suptitle(f"External comparison within the study area (Spearman \u03c1 = {float(_rho):.2f})", fontsize=11)
