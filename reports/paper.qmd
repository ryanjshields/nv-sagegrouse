...
to occur at higher elevations (model-averaged β = 0.94, 85% CI 0.86 – 1.03)
and less likely to occur on steeper slopes (β = −1.17, 85% CI −1.31 – −1.03)
and on rugged terrain measured by the Vector Ruggedness Measure (β = −1.00,
85% CI −1.26 – −0.74); herbaceous cover types were favored over conifer,
which was strongly avoided, and leks sat modestly closer to roads than
availability (β = −0.09, 85% CI −0.17 – −0.01). The averaged model discriminated well (AUC =
...
found no pairwise correlation more extreme than |0.6| (Fig. 3). We then generated 12
a priori models (Table 3) using different combinations of the covariates.
...
with the area under the ROC curve on the fitted model average and under
5-fold cross-validation of the top model, and calibration was inspected by
...
higher elevations (β = 0.94, 85% CI 0.86 – 1.03) and less likely to occur
on steeper slopes (β = −1.17, 85% CI −1.31 – −1.03) and on rugged terrain
(VRM; β = −1.00, 85% CI −1.26 – −0.74): used lek sites averaged VRM 0.00046
...
surface, the continuous Boyce index was 0.84, with the predicted-to-expected
ratio rising from 0.4 in the lowest predicted-probability class to 7.7 in
the highest (S2 Table).
...
With all road classes — dominated by unimproved and local roads — included
in the distance surface, we found a modest association between leks and
road proximity. This is consistent with lekking activity favoring open and
accessible sites [1, 7], and with the observation that unpaved, remotely
located roads can host successful lek sites while busier, noisier roads may
be inhospitable to lekking activity [13]; the class-specific structure of
this association (openness of two-track country versus tolerance of low-use
disturbance) is separable with class-specific distance models, which we
identify as the immediate next analysis for this dataset.
