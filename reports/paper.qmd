...
format:
  ...
  typst:
    ...
    fontsize: 10.5pt
    include-before-body:
      - text: "#set figure(numbering: none)"
---
...
Sage-grouse are a lekking bird species: males gather each spring at
traditional communal display grounds — leks — situated in open areas, and
breeding is concentrated at these sites [1, 7]. Because sage-grouse congregate at lek sites during the
...
attribute (EVT_PHYS), which groups vegetation by growth form (e.g.,
shrubland, grassland, conifer) rather than by floristic composition.
...
classified under the MAF/TIGER Feature Class Code (MTFCC)
schema. County-level road files include every
...
included aspect, slope, curvature, and ruggedness (Fig. 2). Curvature is total
surface curvature — the sum of profile and tangential components, in the
sign convention of the ArcGIS Curvature tool — where positive values
indicate locally convex-upward terrain (ridges, knolls) and negative values
indicate concave terrain (swales, drainages). We elected to use a 30-m
...
roads and unimproved vehicular trails (Table 1, Table 2; Figs. 3–4).

![Figure 3. Road length in the study area by MTFCC class.](v4/figs/roads_by_class.png){width=70%}

![Figure 4. Road length in the study area by usage group.](v4/figs/roads_by_usage.png){width=65%}
...
0.6 (Fig. 5). (Distance to any road and distance to unpaved roads are
...
using the full average in MuMIn, in which a covariate absent from a model
contributes zero to that model's term rather than being omitted, and report
85% confidence intervals, the interval width consistent with AIC-based
model selection [43] (Table 4). Model discrimination was assessed
with the area under the receiver operating characteristic (ROC) curve
(AUC) — the probability that a randomly chosen used site receives a higher
predicted value than a randomly chosen available site, where 0.5 indicates
discrimination no better than chance and 1 indicates perfect ranking — on
the fitted model average, under random
...
use–availability designs: a moving window is swept across the range of
predicted probabilities (ten overlapping classes, each spanning one-fifth
of the range [42]), and each class's predicted-to-expected (P/E) ratio is
computed as the proportion of used leks falling in the class divided by
the proportion of the study area the class occupies; because the windows
overlap, adjacent classes deliberately share observations. The index
...
locations. That product classes the landscape into four percentile
thresholds of a habitat-selection index — non-habitat (below the 5th
percentile), low (5th–25th), moderate (25th–50th), and high (above the
50th) — classes of deliberately unequal area, so we compare against our
five equal-area quantile bins ordinally (rank correlation and a
row-normalized cross-tabulation) rather than class-for-class.
...
quantile classification. Because the used:available ratio is fixed
by design, fitted values are relative indices of selection rather than
absolute probabilities of lek occurrence [41]; we accordingly interpret the
surface through its ranks — quantile classes, rank correlations, and the
Boyce index — rather than through the magnitude of any single predicted
value. To assess sensitivity to post-2018
...
![Figure 5. Pearson correlations among continuous
...
on slope, ruggedness, and elevation before any model was fit (Figs. 6–7).

![Figure 6. Continuous predictor distributions at used and available
sites.](v4/figs/distributions.png)

![Figure 7. Used versus available sites, box-and-whisker (whiskers 1.5×IQR,
...
![Figure 8. Model-averaged coefficients with 85% CIs (forest plot). Red indicates
negative estimates and blue positive; open symbols mark effects whose 85%
CI crosses zero. Vegetation (Veg:) rows are contrasts relative to
shrubland; aspect rows are contrasts relative to east-facing
slopes.](v4/figs/forest.png){width=70%}

![Figure 9. Marginal response curves from the top model: predicted probability of lek
occurrence across each covariate's observed range, others held at
reference values.](v4/figs/response_curves.png)

Based on the model-averaged results (Table 4; Figs. 8–9), leks were more
likely to occur at
...
![Figure 10. Vegetation physiognomy at used versus available
...
(AUC = 0.81, Fig. 11; random 5-fold cross-validation 0.80 ± 0.01; spatially blocked
5-fold cross-validation of the top model 0.80 ± 0.04), and predicted
values tracked observed use rates across deciles (Fig. 12) — internally
consistent, noting that fitted values are relative selection indices rather
than absolute occurrence probabilities. Evaluated
against the final prediction surface, the continuous Boyce index was 0.92
(Fig. 13; S2 Table): lek density rises nearly monotonically across the
predicted-suitability gradient, and cells in the highest probability class
hold leks at roughly six times the rate their area alone would predict. Externally, the surface correlated positively with the USGS
telemetry-based spring habitat selection categories (Fig. 14; Spearman
ρ = 0.53 at ~147,000 common locations): 46% of the cells the USGS classes
as non-habitat fell in our lowest quantile bin and 41% of their
high-habitat cells in our highest — aligned with independent data while
retaining the complementary,
...
![Figure 11. Receiver operating characteristic (ROC) curve for the top
model.](v4/figs/roc.png){width=40%}

![Figure 12. Calibration: mean predicted probability versus observed use
rate by decile.](v4/figs/calibration.png){width=45%}

![Figure 13. Boyce evaluation: predicted-to-expected ratio by probability
class.](v4/figs/boyce_curve.png){width=48%}

![Figure 14. External comparison against the USGS telemetry-based spring
habitat-selection categories, clipped to the study
area.](v4/figs/usgs_side_by_side.png)

**Cross-tabulation** (rows: USGS selection category, row-normalized; cell
shading scales with each row's share): the gradient is monotone — 46% of
USGS non-habitat falls in our lowest bin, and 41% of their high-habitat
cells fall in our highest.
...
{{< include v4/tables/usgs_contingency_plain.md >}}
:::

Predicted probability at used leks was
uncorrelated with peak male counts (Spearman ρ = 0.04), indicating the
surface characterizes lek placement rather than lek size [6]. The
classified surface is mapped statewide in Fig. 15, with each quantile class
isolated in Fig. 16 and a 60-km window resolved at full 30-m detail in
Fig. 17.

![Figure 15. Predicted probabilities of greater sage-grouse lek occurrence in
...
![Figure 16. Each quantile class isolated (equal-area by construction). The
...
![Figure 17. Fine-scale structure of the prediction surface: the statewide map with a
...
- [43] Arnold TW. Uninformative parameters and model selection using
  Akaike's Information Criterion. J Wildl Manage 2010;74(6):1175–1178.
